...
double *jacobi(int device_count) {
    ...
    for (dev_id = 0; dev_id < device_count; ++dev_id) {
        ...
        // Send the first row of the current device to the "top" device
        // Only if the current device isn't already the top one
        if (dev_id > 0) {
            cudaMemcpyAsync(d_a_new[top] + iy_end[top] * WIDTH,
                            d_a_new[dev_id] + (iy_start[dev_id] - 1) * WIDTH,
                            WIDTH * sizeof(double),
                            cudaMemcpyDeviceToDevice);
        }

        // Send the last row of the current device to the "bottom" device
